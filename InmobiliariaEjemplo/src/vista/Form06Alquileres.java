...
public class Form06Alquileres extends JDialog {
	...
	private void cargarTabla() {
		...
		int tam = 0;
		if(controlador.validarNulo(textFieldTamanho.getText())) {
			tam = 0;
		}else {
			tam = Integer.parseInt(textFieldTamanho.getText());
		}
		
		boolean pis = chckbxPiscina.isSelected();
		boolean gar = chckbxGaraje.isSelected();
		boolean jar = chckbxJardin.isSelected();
			
		Zonas zona = (Zonas) comboBox.getSelectedItem();
		
		ArrayList<InmueblesPropietarios> inmuebles = controlador.buscarInmuebles(
				zona.getCodigo(), tam, pis, gar, jar);
	...
	private class OkButtonActionListener implements ActionListener {
		public void actionPerformed(ActionEvent arg0) {
			if(controlador.validarNulo(textFieldContrato.getText()) == true)
				JOptionPane.showMessageDialog(null, "El nº contrato no puede ser nulo");
			else {
				if(controlador.validarNulo(textFieldFecContrato.getText()) == true)
					JOptionPane.showMessageDialog(null, "La fecha del contrato no puede ser nulo");
				else {
					if(controlador.validarNulo(textFieldFecVencimiento.getText()) == true)
						JOptionPane.showMessageDialog(null, "La fecha de vencimiento no puede ser nulo");
					else {
						if(controlador.validarNulo(textFieldPrecio.getText()) == true)
							...
							Contratos contrato = new Contratos(
									...
									ConvertirFechas.convertirStringDate(textFieldFecContrato.getText()),
									ConvertirFechas.convertirStringDate(textFieldFecVencimiento.getText()),
